...
from ivd_utils.util import compute_scale_and_shift, get_interpolate_frames
...
class VideoDepthAnything(nn.Module):
    ...
    def infer_video_depth_one(self, frame, input_size=518, device='cuda', fp32=False):
        ...
        if self.transform is None:  # first frame
            ...
            # Move cache to CPU to save VRAM
            cached_hidden_state_list_cpu = [t.cpu() for t in cached_hidden_state_list]
            self.frame_cache_list = [cached_hidden_state_list_cpu] * INFER_LEN
            ...
            # Move cache back to GPU for inference
            cur_cache = [torch.cat([h[i].to(device) for h in cur_list], dim=1) for i in range(len(cur_list[0]))]
            ...
            # Move new cache to CPU before storing
            new_cache_cpu = [t.cpu() for t in new_cache]
            self.frame_cache_list.append(new_cache_cpu)
